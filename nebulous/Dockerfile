...
# ----------------- EMS Builder image -----------------
FROM docker.io/library/maven:3.9.6-eclipse-temurin-21 AS ems-builder
ENV BASEDIR=/app
WORKDIR ${BASEDIR}
...
COPY --chown=${EMS_USER}:${EMS_USER} --from=ems-builder /app/ems-core/bin              ${BIN_DIR}
COPY --chown=${EMS_USER}:${EMS_USER} --from=ems-builder /app/ems-core/config-files     ${CONFIG_DIR}
COPY --chown=${EMS_USER}:${EMS_USER} --from=ems-builder /app/ems-core/public_resources ${PUBLIC_DIR}
...
COPY --chown=${EMS_USER}:${EMS_USER} --from=ems-builder /app/dependencies          ${BASEDIR}
COPY --chown=${EMS_USER}:${EMS_USER} --from=ems-builder /app/spring-boot-loader    ${BASEDIR}
COPY --chown=${EMS_USER}:${EMS_USER} --from=ems-builder /app/snapshot-dependencies ${BASEDIR}
COPY --chown=${EMS_USER}:${EMS_USER} --from=ems-builder /app/application           ${BASEDIR}

# Copy ESPER dependencies
COPY --chown=${EMS_USER}:${EMS_USER} --from=ems-builder /app/ems-core/control-service/target/esper*.jar       ${BASEDIR}/BOOT-INF/lib/
...
COPY --from=ems-builder /app/ems-nebulous/target/ems-nebulous-plugin-1.0.0-SNAPSHOT-jar-with-dependencies.jar /plugins/
#COPY --from=ems-builder /app/ems-nebulous/models   ${BASEDIR}/models
...
COPY --from=ems-builder ${BUILD_DIR}/banner.txt	                          /tmp/
COPY --from=ems-builder ${BUILD_DIR}/classes/rule-templates.yml	          ${EMS_HOME}/BOOT-INF/classes/
COPY --from=ems-builder ${BUILD_DIR}/classes/metric-model-schematron.xml  ${EMS_HOME}/BOOT-INF/classes/

RUN cat /tmp/banner.txt >> ${EMS_HOME}/BOOT-INF/classes/banner.txt \
#    && rm /tmp/banner.txt \
    && date -Iseconds > /tmp/build.timestamp

COPY --from=ems-builder ${BUILD_DIR}/classes/empty.yml	                ${EMS_HOME}/models/
COPY --from=ems-builder ${BUILD_DIR}/classes/edge.yml		            ${EMS_HOME}/models/
COPY --from=ems-builder ${BUILD_DIR}/classes/extension.yml	            ${EMS_HOME}/models/

COPY --from=ems-builder ${SRC_DIR}/resources/helm/app-detection.yml	    ${EMS_HOME}/config/baguette-client-install/helm/
COPY --from=ems-builder ${SRC_DIR}/resources/helm/epm-deploy.yml        ${EMS_HOME}/config/baguette-client-install/helm/
COPY --from=ems-builder ${SRC_DIR}/resources/helm/netdata-deploy.yml    ${EMS_HOME}/config/baguette-client-install/helm/
COPY --from=ems-builder ${SRC_DIR}/resources/app-ems-server/values.yaml	${EMS_HOME}/config/app-ems-server/
...
COPY --chown=${EMS_USER}:${EMS_USER} --from=ems-builder  /app/ems-core/baguette-client/target/$INSTALLATION_PACKAGE  /tmp
RUN tar zxvf /tmp/$INSTALLATION_PACKAGE -C /opt && rm -f /tmp/$INSTALLATION_PACKAGE
COPY --chown=${EMS_USER}:${EMS_USER} --from=ems-builder  /app/ems-core/baguette-client/conf/* ${EMS_HOME}/conf/
